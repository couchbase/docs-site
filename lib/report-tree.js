...
module.exports.register = function ({ playbook, config }) {
    ...
    this.on('beforePublish', ({ playbook, siteAsciiDocConfig, siteCatalog, uiCatalog, contentCatalog }) => {
        ...
        const makeTag = (origin) => {
            ...
            const startPath = origin.startPath ? `:${origin.startPath}` : ''
        ...
        siteCatalog.addFile({ contents: Buffer.from(page), out: { path: 'doctor/tree.html' } })

        siteCatalog.addFile({ contents: Buffer.from(JSON.stringify(playbook)), out: { path: 'doctor/antora-playbook.json' } })
    })
