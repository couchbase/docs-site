 ...
 *
 * Each `from` name in the map is handled one of two ways, decided automatically
 * by whether it's still a live, registered component in this build:
 *
 * - LIVE (e.g. `analytics`, mid-rename, still producing real content): its
 *   page/image/attachment files are republished under the target URL segment,
 *   without touching the component's registered name - so xrefs/includes against
 *   it keep resolving normally. This is Dan Allen's ContentCatalog#createFile
 *   trick - see remapLiveComponent() below, and
 *   https://antora.zulipchat.com/#narrow/channel/282400-users/topic/.22Fun.22.20renaming.20components/with/619594674
 *
 * - RETIRED (e.g. `columnar`, renamed before this extension existed, with no
 *   content source of its own left in the build): a single whole-prefix redirect
 *   is added, `/<from>/* -> /<to>/*`, with no attempt to distinguish versions -
 *   see addLegacyAlias() below. This is the moral equivalent of
 *   ContentCatalog#addSplatAlias, inlined because addSplatAlias requires a
 *   non-empty version segment on the "from" side (content-catalog.js: "cannot
 *   map splat alias from empty version segment"), which a retired/unversioned
 *   name doesn't have - and since we're redirecting the whole component
 *   regardless of version anyway, we don't need any of its version bookkeeping.
 *
 * Because a name flips from LIVE to RETIRED the moment its content source is
 * dropped from the playbook, no reconfiguration is needed when that happens -
 * e.g. once `analytics` itself is retired in favour of a real `operational-insights`
 * component, this same map entry keeps working, just via the other branch.
 *
 * Redirects for the RETIRED case only take effect for redirect facilities that
  ...
  this.once('contentClassified', ({ contentCatalog }) => {
    for (const [to, froms] of Object.entries(map)) {
      for (const from of Array.isArray(froms) ? froms : [froms]) {
        if (contentCatalog.getComponent(from)) {
          remapLiveComponent(contentCatalog, from, to)
        } else {
