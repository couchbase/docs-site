 ...
 *
 * NOTE this must be `data`, not some other key like `map`: playbook-builder
 * camelCases every key it loads from YAML (so operational-insights-cloud would
 * become operationalInsightsCloud, breaking the URL), except for the one path
 * per extension entry it's explicitly told to leave alone - `data` is that path
 * (see the `preserve: ['data']` entry for antora.extensions in
 * @antora/playbook-builder/lib/config/schema.js). Confirmed empirically: a
 * sibling `map` key on the same entry gets its keys camelCased, `data` doesn't.
 *
...
function remapLiveComponent (contentCatalog, from, to) {
  contentCatalog.getComponent(from).versions.forEach((componentVersion) => {
    ...
    files.forEach((file) => {
      ...
      // file.src.component (which would break xref/include resolution). Passing
      // the real componentVersion through as src.componentVersion is required:
      // otherwise createFile tries to look up a component version for `to`,
      // which isn't a registered component, gets back a bare {version} object
      // with no cached activeVersionSegment or component link, and silently
      // falls back to the raw version number even for the latest version of a
      // component using the redirect:to latest-version-segment strategy - so
      // e.g. the live enterprise-analytics -> operational-insights remap would
      // publish the latest version at /operational-insights/2.2/... instead of
      // /operational-insights/current/..., with nothing left at the /current/
      // URL under the new name at all.
      const mockSrc = Object.assign({}, file.src, { component: to, componentVersion })
