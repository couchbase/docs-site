...
function initSourceUrlIncludeProcessor ({ file, contentCatalog }) {
  return function () {
    ...
    this.process((doc, reader, target, attrs) => {
      ...
      let sourceUrl = (includeFileSrc.fileUri || includeFileSrc.editUrl.replace(`/edit/${includeFileSrc.origin.branch}`, `/blob/${includeFileSrc.origin.refhash}`)) + resolveLineRangeFragment(includeFile.contents, attrs)
