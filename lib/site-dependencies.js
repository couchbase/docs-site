  ...
  antora.once('beforePublish', async ({ siteCatalog }) => {
    ...
    if (localNavFile && localNavJsonFile) {
      try {
        ...
        // orphan-cv versions in the local output aren't really "rebuilt" - they're just
        // whatever Antora's nav-data generator produced from the reduced, post-.out-stripping
        // catalog (often near-empty) - strip them so the merge falls through to the full
        // site's untouched entry instead of overwriting it with impoverished orphan-cv data
        const localNav = stripOrphanVersions(JSON.parse(localNavJsonFile.contents.toString()), orphanCvKeys)
    ...
    if (localManifestFile) {
      try {
        ...
        // same reasoning as the nav merge above - see stripOrphanVersions
        const rawLocalManifest = JSON.parse(localManifestFile.contents.toString())
        const localManifest = { ...rawLocalManifest, components: stripOrphanVersions(rawLocalManifest.components, orphanCvKeys) }
...
// drops any component-version belonging to an orphan cv from a locally-generated nav/manifest
// component list (both share the {name, versions: [{version, ...}]} shape at this level) -
// used before merging, so an orphan cv's version looks like it was never locally produced at
// all, rather than like it was rebuilt down to almost nothing
function stripOrphanVersions (components, orphanCvKeys) {
  return components
    .map((c) => ({ ...c, versions: c.versions.filter((v) => !orphanCvKeys.has(`${c.name}@${v.version}`)) }))
