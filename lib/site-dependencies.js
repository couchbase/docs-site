...
async function pruneUnchangedSources (playbook, fullJson) {
  ...
  const originNote = new Map() // oKey -> 'unchanged' | 'no-baseline' | 'changed-sha' | 'sibling-in-cv:<cvKey>'
  for (const { sha, originKeys } of branchChecks) {
    for (const oKey of originKeys) {
      const recorded = fullJson.origins[oKey]
      const note = !recorded ? 'no-baseline' : !sha || recorded.refhash !== sha ? 'changed-sha' : 'unchanged'
      originNote.set(oKey, note)
      console.log(`quick-build: ${oKey} -> ${note} (recorded=${recorded?.refhash ?? '<none>'}, current=${sha ?? '<none>'})`)
    }
  }
  const directlyChanged = new Set([...originNote].filter(([, note]) => note !== 'unchanged').map(([oKey]) => oKey))
  ...
  for (const source of sources) {
    ...
    for (const { branch, originKeys } of branchEntries) {
      ...
      if (originKeys.some((k) => changed.has(k))) {
        const reason = originKeys.map((k) => originNote.get(k)).find((n) => n && n !== 'unchanged') || 'unknown'
