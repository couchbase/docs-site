    ...
    return function () {
        this.includeProcessor(function () {
            ...
            this.process((doc, reader, target, attrs) => {
                ...
                const src = (cursor.file || {}).src || reader.src
                ...
                if (resolvedTags) {
                    ...
                    const statement = db.prepare("INSERT INTO includeTags(doc_file_path, git_branch, inclusion_path, tag) VALUES (?,?,?,?)")

                    // We need to split up the tags so they are recorded individually.
                    resolvedTags.forEach((_, key) => {
                        const info = statement.run(`${doc.getAttribute('page-edit-url')}`, `${doc.getAttribute('page-origin-branch')}`,includeFileSrc.path,  key)
                    })
                ...
                getTagsFromSource(db, includeFileSrc.path, src, includeContent)
 ...
 * docs build.
 */
function getTagsFromSource(db, path, src, content) {
    ...
    for (const match of matches) {
        ...
        const statement = db.prepare(`
          INSERT INTO sourceTags (source_path, component, module, version, tag)
          VALUES(?,?,?,?,?)
          `)

        statement.run(path, src.component, src.module, src.version, tag)
