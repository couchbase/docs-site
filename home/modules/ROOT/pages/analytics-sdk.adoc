...
The available options for Enterprise Analytics `DataFrame` and `Dataset` operations with the xref:spark-connector::index.html[Couchbase Spark Connector]
can be found on the Spark xref:spark-connector:ROOT:spark-sql.adoc#enterprise-analytics-options[] page.
